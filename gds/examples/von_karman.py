...
def von_karman():
	...
	velocity, pressure = fluid.navier_stokes(G, viscosity=10, v_free=v_free, e_free=e_free, e_normal=e_normal)
	...
	velocity.set_constraints(dirichlet=gds.combine_bcs(
		...
		{e: 0 for e in obstacle_boundary},
		gds.zero_edge_bc(t),
		gds.zero_edge_bc(b),
	))
	...
	gds.render(sys, canvas=gds.grid_canvas(sys.observables.values(), 3), edge_max=0.6, dynamic_ranges=True, edge_palette=cc.bgy)
